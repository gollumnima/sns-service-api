...
// 해당 글의 댓글 목록 조회
router.get('/:post_id', control(async ({ req }) => {
...
// 해당 글의 댓글 등록
// 포스트맨 상에는 바디에 컨텐트만
router.post('/:post_id', checkToken, control(async ({ req }) => {
...
// 댓글 수정
router.put('/:comment_id', checkToken, control(async ({ req }) => {
  const { content } = req.body;
...
router.delete('/:comment_id', checkToken, control(async ({ req }) => {
  const { comment_id } = req.params;
  const { id: user_id } = req.user;
  const [result] = await Comments.update({
    status: 'DELETED',
